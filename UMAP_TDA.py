...
@app.cell
def _(pd):
    taxa_df = pd.read_csv('data/metadata/taxon_annotations.csv', index_col=0)
    taxa_df
    return
...
@app.cell
def _(meta_df, sns, umap_emb):
    df_umap = meta_df[['Year', 'Season', 'Latitude', 'Longitude', 'Water_Temperature', 'Fluorescence']].copy()
    ...
    # sns.scatterplot(data=df_umap, x="UMAP1", y="UMAP2", hue="Year", alpha = 0.5)
    ax = sns.scatterplot(data=df_umap, x="UMAP1", y="UMAP2", hue="Fluorescence", alpha=0.5)
    ax.axis("off")
    ax
    return (df_umap,)
